...
use crate::{render::ChildEffectOf, ParticleLayout};
...
pub(crate) fn on_remove_cached_effect_events(
    ...
    query: Query<&CachedEffectEvents>,
    mut event_cache: ResMut<EventCache>,
) {
    #[cfg(feature = "trace")]
    ...
    if let Ok(cached_effect_event) = query.get(trigger.target()) {
        event_cache.free(cached_effect_event);
    };
}
