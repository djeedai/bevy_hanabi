...
pub enum Value<T: Copy + FromReflect + 'static> {
...
impl<T: Copy + FromReflect + Default + 'static> Default for Value<T> {
    fn default() -> Self {
        Self::Single(T::default())
    }
}

impl<T: Copy + FromReflect + SampleUniform + 'static> Value<T> {
    /// Sample the value.
...
impl<T: Copy + FromReflect + PartialOrd + 'static> Value<T> {
...
impl<T: Copy + FromReflect + 'static> From<T> for Value<T> {
    fn from(t: T) -> Self {
        Self::Single(t)
    }
}
