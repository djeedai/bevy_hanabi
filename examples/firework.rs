...
fn create_rocket_effect() -> EffectAsset {
    ...
    let scale = writer.lit(0.1);
    let x = writer.rand(ScalarType::Float) * scale.clone();
    let y = writer.lit(140.).uniform(writer.lit(160.));
    let z = writer.rand(ScalarType::Float) * scale;
    let v = x.vec3(y, z);
    ...
    let update_spawn_on_die = SpawnEventModifier {
        condition: SpawnEventCondition::OnDie,
        count: 100,
    };
...
fn create_trails_effect() -> EffectAsset {
    ...
    let speed = writer.lit(50.).uniform(writer.lit(80.)).expr();
    ...
    color_gradient.add_key(0.6, Vec4::new(4.0, 0.0, 0.0, 1.0));
